...
<div class="flex flex-wrap gap-3 justify-center">
    ...
    <div class="max-w-sm rounded overflow-hidden shadow-lg bg-white">
        ...
        <div class="px-6 py-4">
            ...
            <p class="text-gray-700 text-base bg-blue-200 rounded-lg px-4 py-0.5 ">{{$projects->name}}</p>
            <p class="text-gray-700 text-base">{{$projects->enterprise->slogan}}</p>
            </div>
            ...
            @if ($projects->freelancers->contains(auth()->user()->freelancer))
                    <p class="text-green-400 ">You have already applied to this project</p>
                    @else
<form action="{{ route('projects.apply', ['projectId' => $projects->id]) }}" method="POST">
    @csrf
    <button type="submit" class=" flex text-blue-600">
    <svg
      class="svg w-8 text-blue-600"
      fill="none"
      height="24"
      stroke="currentColor"
